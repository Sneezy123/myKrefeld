...
export default function MapPage({ events }: MapPageProps) {
    ...
    const updateMarkersAndClusters = () => {
        ...
        const clusterIndex = clusterIndexRef.current as Supercluster<any, any>;
        ...
        clusters.forEach((cluster) => {
            ...
            if (isCluster) {
                ...
                element = clusterElement.firstChild as HTMLElement;
                ...
                createRoot(popupDiv).render(<MarkerPopup cluster={cluster as any} />);
    ...
    useEffect(() => {
        ...
        const features: Supercluster.PointFeature<{ eventId: string; eventTitle: string; }>[] = events
            .filter((event): event is Event & { venue: { lon: number; lat: number; } } => event.venue?.lon !== undefined && event.venue?.lat !== undefined)
            .map((event) => ({
                type: 'Feature',
                geometry: {
                    type: 'Point',
        ...
        clusterIndexRef.current.load(features as Supercluster.PointFeature<any>[]);
