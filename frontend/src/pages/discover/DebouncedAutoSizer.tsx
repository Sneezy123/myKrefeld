...
interface DebouncedAutoSizerProps {
...
function DebouncedAutoSizer({
    ...
    debounceTime = 150,
    disableHeight = false,
}: DebouncedAutoSizerProps) {
    // State to store the *latest* (non-debounced) dimensions from AutoSizer
    const [latestAutoSizerWidth, setLatestAutoSizerWidth] = useState<number>(0);
    const [latestAutoSizerHeight, setLatestAutoSizerHeight] = useState<number>(0);

    // State to store the *debounced* dimensions that will be passed to children
    const [debouncedRenderWidth, setDebouncedRenderWidth] = useState<number>(0);
    const [debouncedRenderHeight, setDebouncedRenderHeight] = useState<number>(0);
