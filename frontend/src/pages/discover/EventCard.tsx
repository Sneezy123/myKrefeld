                ...
                <Card className='w-full h-[560px] transition-all flex flex-col cursor-pointer hover:border-accent-400 duration-300'>
                    ...
                    <CardContent>
                        <p className='font-stretch-semi-expanded text-sm text-text-900  mb-2'>
                            ...
                            <br />
                            <strong>Ende:</strong>{' '}
                            {new Date(event.end_date).toLocaleString('de-DE', {
                                year: 'numeric',
                                month: '2-digit',
                                day: '2-digit',
                                hour: '2-digit',
...
function TimeIndicator({
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(
            ...
            activeNumber === 0 ?
                (getTimeUntilString(currentTime, eventStart)[1] as number)
            :   (getTimeUntilString(currentTime, eventEnd)[1] as number)
        );
...
function getTimeUntilString(now: number, date: number) {
